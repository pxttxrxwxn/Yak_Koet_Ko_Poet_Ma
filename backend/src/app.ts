...
export function createApp(containerFactory: (env: Partial<Bindings>) => Container) {
  ...
  app.get(
    ...
    openAPIRouteHandler(app, {
      documentation: {
        info: {
          title: 'Yak Koet Ko Poet Ma API',
          version: '1.0.0',
          description: 'ระบบสร้างแบบทดสอบก่อนสอบจากไฟล์เรียน — Hono + Cloudflare Workers + D1 + KV',
        },
        tags: [
          { name: 'Users', description: 'User management' },
          { name: 'Quizzes', description: 'Quiz / Exam management' },
          { name: 'QuizAttempts', description: 'Quiz attempt history' },
        ],
      },
    })
  )
  app.get('/docs', Scalar({ url: '/openapi.json', pageTitle: 'Yak Koet Ko Poet Ma API Docs' }))
